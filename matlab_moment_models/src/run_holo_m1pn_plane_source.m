...
orders = cfg.holo.pn_orders;
rows_cell = cell(numel(orders), 1);
run_cell = cell(numel(orders), 1);

ens_ctx = mm_parallel_context(parallel_cfg, numel(orders), 'ensemble');
use_ensemble = ens_ctx.use_parallel;

if use_ensemble
    parfor i = 1:numel(orders)
        N = orders(i);
        out = run_single_holo_order(N, cfg, gridData, z, dz, nCells, rhoRef, timeRef, tCfg, parallel_cfg);
        rows_cell{i} = out.row;
        run_cell{i} = out;
    end
else
...
if use_ensemble && tCfg.print_summary
    for i = 1:numel(run_cell)
        print_holo_summary(run_cell{i}.row);
    end
end

for i = 1:numel(run_cell)
    N = run_cell{i}.row.order;
    rho = run_cell{i}.rho;
    rho_m1 = run_cell{i}.rho_m1;

    outProf = fullfile(cfg.paths.results, sprintf('paper2_plane_source_HOLOM1PN_%d_profile.csv', N));
    Tprof = table(z(:), rho(:), rho_m1(:), rhoRef(:), ...
...
function out = run_single_holo_order(N, cfg, gridData, z, dz, nCells, rhoRef, timeRef, tCfg, parallel_cfg)
...
function print_holo_summary(row)
fprintf('[timing] HOLOM1PN-%d: total=%.3fs solver=%.3fs source_pn=%.3fs source_m1=%.3fs flux=%.3fs pn_lim+flux=%.3fs m1_lim+flux=%.3fs sync=%.3fs\n', ...
    row.order, row.runtime_s, row.time_step_sum_s, row.time_source_pn_sum_s, row.time_source_m1_sum_s, row.time_flux_sum_s, ...
    row.time_flux_pn_limiter_flux_sum_s, row.time_flux_m1_limiter_flux_sum_s, row.time_flux_sync_sum_s);
end
