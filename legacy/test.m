...
% --------- Live-Plot vorbereiten (rho) ----------
figure;
rho_init = u(1,:);                     % Anfangsdichte
h_rho    = plot(z, rho_init, 'LineWidth', 1.5);
xlabel('x'); ylabel('\rho');
title('Entwicklung von \rho(x,t)');
grid on;
ylim([min(rho_init)-0.1*max(rho_init), 1.1*max(rho_init)]);
drawnow;
% -----------------------------------------------
...
for t = 0:dt:1
    ...
    % Gleichung (20): rho^{n+1}
    rho = w.'*cd;

    % Gleichung (17): v^{n+1}
    v = gamma*cd + (1-gamma)*rho/2;

    % ---- Live-Plot updaten (alle paar Schritte) ----
    if mod(round(t/dt), 10) == 0      % z.B. jede 10. Zeitscheibe
        set(h_rho, 'YData', rho);     % neue Dichte eintragen
        drawnow limitrate;
    end
    % ------------------------------------------------
end
