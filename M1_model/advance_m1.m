function u = advance_m1(un, dt, dx, f, fj, Nz, sigma_a, sigma_s, method)
    ...
    u1  = un + dt .* (-(f1 - f1(:,im1c)) / dx + source_term(Nz) - reaction(un, sigma_a, sigma_s));

    % Stufe 2
    psi2_1 = calc_psi2(u1);
    f2  = method(u1, dt, dx, f, lambda, psi2_1, Nz);
    u2 = u1 + dt .* (-(f2 - f2(:,im1c)) / dx + source_term(Nz) - reaction(u1, sigma_a, sigma_s));
