...
%[uplus, uminus] = polynomial_reconstruction(u,ub);

% Positive und negative Flüsse berechnen für (4.13) mit der Formel (4.15
if strcmpi(opts.model,'PN')
    [Fpos, Fneg] = flux_parts_PN(u, opts.N, opts.quad, Ppos, Pneg, wmu_pos, wmu_neg);
    %[Fpos, ~] = flux_parts_PN(uminus, opts.N, opts.quad, Ppos, Pneg, wmu_pos, wmu_neg);
    %[~, Fneg] = flux_parts_PN(uplus, opts.N, opts.quad, Ppos, Pneg, wmu_pos, wmu_neg);
else
